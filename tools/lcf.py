...
with open(f'{BUILD}arm9_linker_script.lcf', 'w') as file:
    ...
    for obj in ITCM_OBJECTS: file.write(f'        {name(obj)}(.text)\n')
    for obj in ITCM_OBJECTS: file.write(f'        {name(obj)}(.rodata)\n')
    file.write('        itcm_end = .;\n')
    ...
    for obj in DTCM_OBJECTS: file.write(f'        {name(obj)}(.data)\n')
    file.write('        dtcm_end = .;\n')
    ...
    for ov in OVERLAYS:
        ...
        for obj in ov.objects: file.write(f'        {name(obj)}(.text)\n')
        for obj in ov.objects: file.write(f'        {name(obj)}(.rodata)\n')
        for obj in ov.objects: file.write(f'        {name(obj)}(.init)\n')
        file.write('\n')
        file.write(f'        {ov.name}_sinit_start = .;\n')
        for obj in ov.objects: file.write(f'        {name(obj)}(.sinit)\n')
        file.write('        WRITEW 0;\n')
        file.write(f'        {ov.name}_sinit_end = .;\n')
        file.write('\n')
        file.write('        . = ALIGN(32);\n')
        for obj in ov.objects: file.write(f'        {name(obj)}(.data)\n')
